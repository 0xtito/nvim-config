--
-- Follow Markdown Links (inline, no plugin manager needed)
--
...
local function get_inline_node_at_cursor(row, col)
  ...
  if not inline_node then
    return nil
  end

  return inline_root:named_descendant_for_range(row, col, row, col)
end
  ...
  elseif node_at_cursor:type() == 'link_reference_definition' or node_at_cursor:type() == 'inline_link' then
    for i = 0, node_at_cursor:named_child_count() - 1 do
      ...
      if child:type() == 'link_destination' then
        return vim.split(treesitter.get_node_text(child, bufnr), '\n')[1]
      end
    end
  elseif node_at_cursor:type() == 'full_reference_link' then
    for i = 0, node_at_cursor:named_child_count() - 1 do
      ...
      if child:type() == 'link_label' then
        local link_label = vim.split(treesitter.get_node_text(child, bufnr), '\n')[1]
        return get_reference_link_destination(link_label)
      end
    end
...
local function follow_local_link(link)
  ...
  -- attempt to add .md extension if file doesn't exist
  local modified_link
  ...
  if line_number then
    cmd(string.format('e +%s %s', line_number, fn.fnameescape(modified_link)))
  else
    cmd(string.format('e %s', fn.fnameescape(modified_link)))
  end
end
...
vim.api.nvim_create_autocmd('FileType', {
  ...
  callback = function(ev)
    vim.keymap.set('n', '<CR>', follow_link, { buffer = ev.buf, desc = 'Follow markdown link' })
  end,
})
